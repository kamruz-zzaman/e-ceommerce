...
import { CATALOG_SEED, FAMILIES, REVIEW_AUTHORS, REVIEW_CONTEXTS } from "./catalog-definitions.ts";
...
function makeReviews(key: string, family: FamilyDefinition): readonly Review[] {
  ...
  const ratings: readonly Review["rating"][] = profile < 12 ? [2, 3, 3, 2, 4] : profile < 35 ? [3, 4, 3, 4, 4] : [4, 5, 4, 5, 4];
  const contexts = [...REVIEW_CONTEXTS[family.category]].sort((a, b) =>
    choice(key, `context:${a}`, 0xffffffff) - choice(key, `context:${b}`, 0xffffffff));
  const authors = [...REVIEW_AUTHORS].sort((a, b) =>
    choice(key, `author:${a}`, 0xffffffff) - choice(key, `author:${b}`, 0xffffffff));
  return Array.from({ length: count }, (_, index) => {
    const reviewKey = `${key}|context:${contexts[index]}`;
    const rating = ratings[choice(reviewKey, "rating", ratings.length)]!;
    return {
      id: `review-${createHash("sha256").update(reviewKey).digest("hex").slice(0, 16)}`,
      authorName: authors[index]!,
      rating,
      body: `${rating <= 3 ? family.reviewNotes[2] : family.reviewNotes[choice(reviewKey, "observation", 2)]} ${contexts[index]}`,
    };
  });
}
...
function generate(): readonly Product[] {
  ...
  for (const family of FAMILIES) {
    ...
    for (const configuration of family.configurations) {
      ...
      const reviews = makeReviews(key, family);
